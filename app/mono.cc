...
public:
  ...
  void GrabImage(const std::vector<cv::Mat> &image);
...
void printall(ORB_SLAM2::System *sys, Eigen::Vector3f tran,
              Eigen::Quaternionf rot, [[maybe_unused]] double dt) {
  mtransform << std::chrono::duration<double>(
...
int main(int argc, char **argv) {
  ...
  // process frames. ORB_SLAM2::System
...
void ImageGrabber::GrabImage(const std::vector<cv::Mat> &image) {
  if (image.size() == 2) {
    mpSLAM->TrackStereo(
        image[0], image[1],
    ...
    mpSLAM->TrackMonocular(
        image[0], std::chrono::duration<double>(
                      std::chrono::high_resolution_clock::now() - start_time)
                      .count());
  }
}
